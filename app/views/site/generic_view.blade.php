									...
									if(in_array($deployment->status, array('Completed', 'start', 'stop')))
										{
											...
											echo $result->instance_id .xDockerEngine::getDockerUrl($deployment->docker_name) . ' | ' .$anchor . ' | '  .xDockerEngine::documentationUrl($deployment->docker_name) .'<br/>';
											...
											echo '<a title="Download" href="#" onclick="downloadKey(\''.$url.'\',\''.$deployment->id.'\', \''.csrf_token().'\')"><span class="glyphicon glyphicon-download"> </span>Download Pem</a>';
