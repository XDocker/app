									...
									if(in_array($deployment->status, array('Completed', 'start', 'stop')))
										{
											...
											$anchor = '<a target="_blank" href="'.xDockerEngine::getProtocol($deployment->docker_name). $result->public_dns .'">'.xDockerEngine::getDisplayName($deployment->docker_name).xDockerEngine::urlAppend($deployment->name).'</a>';
